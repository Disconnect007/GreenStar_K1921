...
#define MTIMER_SECONDS_TO_CLOCKS(SEC)           \
    (uint64_t)(SEC)*(uint64_t)(MTIME_FREQ_HZ)

#define MTIMER_MSEC_TO_CLOCKS(MSEC)           \
    (((uint64_t)(MSEC) * (uint64_t)(MTIME_FREQ_HZ)) / 1000ULL)

#define MTIMER_USEC_TO_CLOCKS(USEC)           \
	(((uint64_t)(USEC) * (uint64_t)(MTIME_FREQ_HZ)) / 1000000ULL)
...
void mtimer_sleep(uint32_t ms);
void mtimer_usleep(uint32_t us);
